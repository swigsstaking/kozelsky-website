...
const ServicesV4 = () => {
  ...
  const content = {
    fr: {
      ...
      services: [
        { icon: Utensils, title: 'Gastronomie', desc: 'Menus sur-mesure créés selon vos envies', features: ['Cuisine française', 'Influences internationales', 'Options végétariennes', 'Allergènes pris en compte'] },
        { icon: Users, title: 'Personnel', desc: 'Une équipe professionnelle à votre service', features: ['Chefs expérimentés', 'Service en salle', 'Coordination', 'Tenue soignée'] },
        { icon: Truck, title: 'Logistique', desc: 'Transport et installation inclus', features: ['Livraison', 'Mise en place', 'Matériel fourni', 'Nettoyage'] },
        { icon: Clock, title: 'Flexibilité', desc: 'Adaptation à tous vos besoins', features: ['Horaires flexibles', 'Dernière minute', 'Modifications possibles', 'Conseil personnalisé'] },
      ],
      ...
      pricingItems: ['Le nombre de convives', 'Le type de menu choisi', 'Les services additionnels', 'La distance et la logistique'],
    ...
    de: {
      ...
      services: [
        { icon: Utensils, title: 'Gastronomie', desc: 'Maßgeschneiderte Menüs nach Ihren Wünschen', features: ['Französische Küche', 'Internationale Einflüsse', 'Vegetarische Optionen', 'Allergene berücksichtigt'] },
        { icon: Users, title: 'Personal', desc: 'Ein professionelles Team zu Ihren Diensten', features: ['Erfahrene Köche', 'Saalservice', 'Koordination', 'Gepflegte Kleidung'] },
        { icon: Truck, title: 'Logistik', desc: 'Transport und Aufbau inklusive', features: ['Lieferung', 'Aufbau', 'Material bereitgestellt', 'Reinigung'] },
        { icon: Clock, title: 'Flexibilität', desc: 'Anpassung an all Ihre Bedürfnisse', features: ['Flexible Zeiten', 'Kurzfristig möglich', 'Änderungen möglich', 'Persönliche Beratung'] },
      ],
      ...
      pricingItems: ['Die Anzahl der Gäste', 'Die Art des gewählten Menüs', 'Zusätzliche Dienstleistungen', 'Entfernung und Logistik'],
    ...
    en: {
      ...
      services: [
        { icon: Utensils, title: 'Gastronomy', desc: 'Custom menus created to your wishes', features: ['French cuisine', 'International influences', 'Vegetarian options', 'Allergens considered'] },
        { icon: Users, title: 'Staff', desc: 'A professional team at your service', features: ['Experienced chefs', 'Room service', 'Coordination', 'Smart attire'] },
        { icon: Truck, title: 'Logistics', desc: 'Transport and setup included', features: ['Delivery', 'Setup', 'Equipment provided', 'Cleaning'] },
        { icon: Clock, title: 'Flexibility', desc: 'Adapting to all your needs', features: ['Flexible hours', 'Last minute', 'Changes possible', 'Personalized advice'] },
      ],
      ...
      pricingItems: ['The number of guests', 'The type of menu chosen', 'Additional services', 'Distance and logistics'],
  ...
  return (
    <>
      ...
      {/* Hero */}
      <section className="relative py-40 bg-gradient-to-br from-secondary-900 via-secondary-800 to-secondary-900 text-white overflow-hidden">
        <div className="absolute inset-0 opacity-10">
          <div className="absolute top-0 left-0 w-[500px] h-[500px] bg-accent-500 rounded-full filter blur-3xl" />
          <div className="absolute bottom-0 right-0 w-[500px] h-[500px] bg-primary-500 rounded-full filter blur-3xl" />
        </div>
        <div className="container-site relative z-10">
          <div className="max-w-3xl">
            <p className="text-accent-400 uppercase tracking-[0.3em] text-sm mb-8">{c.ourServices}</p>
        ...
        <div className="container-site">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
            {c.services.map((service, index) => (
              <div key={index} className="bg-white p-12 rounded-3xl hover:shadow-2xl transition-shadow">
                <div className="w-20 h-20 bg-secondary-900 rounded-full flex items-center justify-center mb-8">
                  <service.icon size={32} className="text-white" />
                </div>
                <h3 className="text-3xl font-serif text-secondary-900 mb-5">{service.title}</h3>
                <p className="text-secondary-600 mb-8 text-lg">{service.desc}</p>
      ...
      <section className="py-32 bg-white">
        <div className="container-site">
          <div className="text-center mb-20">
            <p className="text-accent-600 uppercase tracking-[0.3em] text-sm mb-6">{c.forAllEvents}</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {c.eventTypes.map((event, index) => (
              <div
                key={index}
                className="group relative p-10 bg-secondary-900 text-white rounded-3xl hover:bg-accent-600 transition-colors cursor-pointer"
              >
                ...
                <ArrowRight size={24} className="absolute bottom-10 right-10 opacity-0 group-hover:opacity-100 transition-opacity" />
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-32 bg-secondary-900 text-white">
        <div className="container-site">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
            <div>
              <p className="text-accent-400 uppercase tracking-[0.3em] text-sm mb-8">{c.pricing}</p>
              <h2 className="text-5xl font-serif mb-10">
                {c.pricingTitle}
              </h2>
              <p className="text-secondary-300 leading-relaxed text-lg mb-8">
                {c.pricingDesc}
              </p>
              <ul className="space-y-4 text-secondary-300">
                {c.pricingItems.map((item, index) => (
                  <li key={index} className="flex items-center gap-4">
                    <span className="w-3 h-3 bg-accent-400 rounded-full" />
                    {item}
                  </li>
                ))}
              </ul>
            </div>
            <div className="bg-white/10 p-12 rounded-3xl backdrop-blur">
              <h3 className="text-3xl font-serif mb-8">{c.freeQuote}</h3>
              <p className="text-secondary-300 mb-10 text-lg">
                {c.quoteDesc}
              </p>
